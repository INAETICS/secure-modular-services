...
public final class ZookeeperEndpointDiscovery extends AbstractHttpEndpointDiscovery<ZookeeperDiscoveryConfiguration>
    ...
    private volatile Set<String> m_discoveryEndpointIds = new HashSet<String>();
    ...
    private void updateDiscoveryEndpoints(List<String> discoveryEndpointIDs) throws Exception {
        ...
        Map<String, URL> updatedDiscoveryEndpoints = new HashMap<String, URL>();
        for (String discoveryEndpointID : discoveryEndpointIDs) {
            try {
                ...
                updatedDiscoveryEndpoints.put(discoveryEndpointID, new URL(new String(bytes)));
            }
            catch (Exception e) {
        ...
        synchronized (m_discoveryEndpointIds) {
            Set<String> updatedDiscoveryEndpointIds = updatedDiscoveryEndpoints.keySet();
            for (String discoveryEndpointId : m_discoveryEndpointIds) {
                if (!updatedDiscoveryEndpointIds.contains(discoveryEndpointId)) {
                    removeDiscoveryEndpoint(discoveryEndpointId);
                }
            }

            for (Map.Entry<String, URL> updatedDiscoveryEndpoint : updatedDiscoveryEndpoints.entrySet()) {
                if (!m_discoveryEndpointIds.contains(updatedDiscoveryEndpoint.getKey())) {
                    addDiscoveryEndpoint(updatedDiscoveryEndpoint.getKey(), updatedDiscoveryEndpoint.getValue());
                }
            }
            m_discoveryEndpointIds = updatedDiscoveryEndpoints.keySet();
        }
    }
}
