...
public class Activator extends DependencyActivatorBase implements EtcdDiscoveryConfiguration, ManagedService {
    ...
    public void updated(Dictionary<String, ?> properties) throws ConfigurationException {
        ...
        try {
            ...
            String connectString = getConfiguredConnectUrl(properties);
            String rootPath = getConfiguredRootPath(properties);

            m_connectTimeout = connectTimeout;
            m_readTimeout = readTimeout;

            if (!baseUrl.equals(m_baseUrl) || m_schedule != schedule || !m_connectUrl.equals(connectString)
                || !m_rootPath.equals(rootPath)) {
                unregisterDiscoveryService();
                m_baseUrl = baseUrl;
                m_schedule = schedule;
                m_connectUrl = connectString;
                m_rootPath = rootPath;
                if (!m_connectUrl.equals("")) {
                    registerDiscoveryService();
                }
    ...
    private void registerDiscoveryService() {
        ...
        Properties properties =
            createEndpointListenerServiceProperties(m_manager.getBundleContext(),
                EtcdEndpointDiscovery.DISCOVERY_TYPE);

        EtcdEndpointDiscovery discovery =
            new EtcdEndpointDiscovery(this);
