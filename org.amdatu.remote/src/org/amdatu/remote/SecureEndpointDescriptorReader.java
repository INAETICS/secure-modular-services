...
public final class SecureEndpointDescriptorReader {
    ...
    public List<EndpointDescription> parseDocument(Reader reader, HttpEndpointDiscoveryConfiguration m_configuration) {
        SecureEndpointDescriptorParserHandler s_handler = new SecureEndpointDescriptorParserHandler();
        EndpointDescriptorParserHandler handler = new EndpointDescriptorParserHandler();
        InputSource source = null;
        try {
            String input = "";
            int valOfChar;
            while ((valOfChar = reader.read()) != -1) {
                input += (char) valOfChar;
            }
            reader.close();
            if(input != null) {
                input = m_configuration.decrypt(input);
            }
    ...
    static class SecureEndpointDescriptorParserHandler extends DefaultHandler {
