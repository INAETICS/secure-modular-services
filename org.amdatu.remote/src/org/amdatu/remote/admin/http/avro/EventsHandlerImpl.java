...
public final class EventsHandlerImpl extends AbstractComponentDelegate {
    ...
    public EventsHandlerImpl(final RemoteServiceAdminFactory adminManager) {
        super(adminManager);
    }

    // Dependency Manager callback method
    protected final void listenerAdded(final ServiceReference<?> reference, final RemoteServiceAdminListener listener) {
        logDebug("RSA listener added %s - %s", reference, listener);
        m_listeners.put(reference, listener);
    }

    // Dependency Manager callback method
    protected final void listenerRemoved(final ServiceReference<?> reference,
        final RemoteServiceAdminListener listener) {
        logDebug("RSA listener removed %s - %s", reference, listener);
        m_listeners.remove(reference);
    }

    // Dependency Manager callback method
    protected final void eventAdminAdded(final ServiceReference<?> reference, final EventAdmin eventAdmin) {
        logDebug("EventAdmin added %s", reference);
        m_admins.put(reference, eventAdmin);
    }

    // Dependency Manager callback method
    protected final void eventAdminRemoved(final ServiceReference<?> reference, final EventAdmin eventAdmin) {
    ...
    public void emitEvent(final int type, final Bundle source, final ExportReference exportRef,
    ...
    public void emitEvent(final int type, final Bundle source, final ImportReference importRef,
        final Throwable exception) {
    ...
    private void emitRemoteServiceAdminEvent(final RemoteServiceAdminEvent event, final EndpointDescription endpoint) {
        EndpointPermission permission = new EndpointPermission(endpoint, getFrameworkUUID(), EndpointPermission.READ);
    ...
    private void emitEventAdminEvent(final Event event) {
        try {
            // AMDATURS-84: make sure we only check permissions when there's a
    ...
    /**
     * Map a Remote Service Admin Event to an EventAdmin event according to OSGi
     * Enterprise R5 122.7.1.
     *
     * @param event
     *        the Remote Service Admin event
     * @return the Event Admin event
     */
    private Event createEventAdminEvent(final RemoteServiceAdminEvent event, final EndpointDescription description,
        final Throwable exception) {
    ...
    private static String getEventTopic(final int type) {
        return EVENT_TOPIC_BASE + getEventName(type);
    }

    private static String getEventName(final int type) {
        switch (type) {
    ...
    private static void putIfValueNotNull(final Map<String, Object> properties, final String key, final Object value) {
        if (value != null) {
