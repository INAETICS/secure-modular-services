...
public final class ImportedEndpointImpl implements ImportRegistration, ImportReference, ClientEndpointProblemListener {
     ...
     */
    public ImportedEndpointImpl(final RemoteServiceAdminImpl admin, final EndpointDescription description,
        final HttpAdminConfiguration configuration) {
    ...
    @Override
    public void handleEndpointError(final Throwable exception) {
        m_admin.getEventsHandler().emitEvent(IMPORT_ERROR, m_admin.getBundleContext().getBundle(), this, exception);
    }

    @Override
    public void handleEndpointWarning(final Throwable exception) {
    ...
    @Override
    public boolean update(final EndpointDescription endpoint) {
        if (m_closed.get()) {
    ...
    void close(boolean ignoreClosed) {
        if (!m_closed.compareAndSet(false, true) && !ignoreClosed) {
            return;
        }
        ServiceRegistration<?> clientRegistration = m_clientRegistration;
    ...
    EndpointDescription getImportedEndpoint(final boolean ignoreClosed) {
    ...
    ServiceReference<?> getExportedService(final boolean ignoreClosed) {
        if (!ignoreClosed && m_closed.get()) {
            return null;
        }
    ...
    Throwable getException(final boolean ignoreClosed) {
        if (!ignoreClosed && m_closed.get()) {
            return null;
        }
        return m_exception;
     ...
     */
    private static String[] createImportedServiceObjectClass(final EndpointDescription description) {
     ...
     */
    private static Dictionary<String, Object> createImportedServiceProperties(
