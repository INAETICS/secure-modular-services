...
import static org.amdatu.remote.admin.http.avro.HttpAdminConstants.CONFIGURATION_TYPE;
import static org.amdatu.remote.admin.http.avro.HttpAdminConstants.ENDPOINT_URL;
import static org.amdatu.remote.admin.http.avro.HttpAdminConstants.PASSBYVALYE_INTENT;
import static org.amdatu.remote.admin.http.avro.HttpAdminConstants.SUPPORTED_INTENTS;
...
public final class RemoteServiceAdminImpl extends AbstractComponentDelegate implements RemoteServiceAdmin {
    ...
    public RemoteServiceAdminImpl(final RemoteServiceAdminFactory manager, final HttpAdminConfiguration configuration) {
        super(manager);
        m_manager = manager;
    ...
    public Collection<ExportRegistration> exportService(final ServiceReference<?> reference,
        ...
        if (securityManager != null) {
            securityManager.checkPermission(new EndpointPermission(endpoint, getFrameworkUUID(), EXPORT));
        }
        ...
        return AccessController.doPrivileged(new PrivilegedAction<Collection<ExportRegistration>>() {
            ...
            public Collection<ExportRegistration> run() {
                ...
                synchronized (m_exportedEndpoints) {
                    ...
                    HttpAvroServerEndpoint serverEndpoint =
    ...
    public ImportRegistration importService(final EndpointDescription endpoint) {
        ...
        SecurityManager securityManager = System.getSecurityManager();
        if (securityManager != null) {
            securityManager.checkPermission(new EndpointPermission(endpoint, getFrameworkUUID(), IMPORT));
        }
    ...
    void addExportedEndpoints(final Collection<ExportReference> collection) {
    ...
    void addImportedEndpoints(final Collection<ImportReference> collection) {
    ...
    void exportedEndpointUpdated(final ExportedEndpointImpl exportedEndpoint) {
        ...
        synchronized (m_exportedEndpoints) {
            Set<ExportedEndpointImpl> exportedEndpoints = m_exportedEndpoints.get(endpoint);
            if (exportedEndpoints != null) {
                updated = exportedEndpoints.remove(exportedEndpoint);
                if (updated) {
                    exportedEndpoints.add(exportedEndpoint);
                }
            }
    ...
    void exportedEndpointClosed(final ExportedEndpointImpl exportedEndpoint) {
        ...
        synchronized (m_exportedEndpoints) {
            Set<ExportedEndpointImpl> exportedEndpoints = m_exportedEndpoints.get(endpoint);
            if (exportedEndpoints != null) {
                removed = exportedEndpoints.remove(exportedEndpoint);
                if (removed && exportedEndpoints.isEmpty()) {
    ...
    void importedEndpointUpdated(final ImportedEndpointImpl importedEndpoint) {
        ...
        synchronized (m_importedEndpoints) {
            Set<ImportedEndpointImpl> importedEndpoints = m_importedEndpoints.get(endpoint);
            if (importedEndpoints != null) {
                updated = importedEndpoints.remove(importedEndpoint);
                if (updated) {
                    importedEndpoints.add(importedEndpoint);
                }
            }
    ...
    void importedEndpointClosed(final ImportedEndpointImpl importedEndpoint) {
        ...
        synchronized (m_importedEndpoints) {
            Set<ImportedEndpointImpl> importedEndpoints = m_importedEndpoints.get(endpoint);
            if (importedEndpoints != null) {
                removed = importedEndpoints.remove(importedEndpoint);
                if (removed && importedEndpoints.isEmpty()) {
    ...
    HttpAvroServerEndpointHandler getServerEndpointHandler() {
        return m_manager.getServerEndpointHandler();
    }

    EndpointDescription createEndpointDescription(final ServiceReference<?> reference,
        final Map<String, ?> extraProperties) {
        return createEndpointDescription(UUID.randomUUID().toString(), reference, extraProperties);
    }

    EndpointDescription createEndpointDescription(final String endpointId, final ServiceReference<?> reference,
        final Map<String, ?> extraProperties) {
        return createEndpointDescription(endpointId, getMergedProperties(reference, extraProperties));
    }

    private Class<?>[] loadEndpointInterfaces(final ServiceReference<?> reference, final EndpointDescription endpoint) {
    ...
    private EndpointDescription createEndpointDescription(final String endpointId,
        ...
        String[] exportedInterfaces = getCheckedExportedInterfaces(properties);
        ...
        properties.put(ENDPOINT_FRAMEWORK_UUID, getFrameworkUUID());
     ...
     */
    private String[] getCheckedExportedInterfaces(final Map<String, ?> exportProperties) {
     ...
     */
    private static String[] getExportedIntents(final Map<String, Object> properties) {
     ...
     */
    private static Map<String, Object> getMergedProperties(final ServiceReference<?> reference,
        final Map<String, ?> extraProperties) {
     ...
     */
    private static boolean isExportedIntentsSupported(final String[] exportedIntents) {
        if (exportedIntents == null) {
            return false;
        }
     ...
     */
    private static boolean hasValidServiceLocation(final EndpointDescription endpoint) {
     ...
     * @return {@code true} if valid, {@code false} otherwise.
     */
    private static boolean hasAvailableInterfaces(final EndpointDescription endpoint) {
    ...
    private static Map<String, Class<?>> collectInterfaces(final Class<?> clazz) {
        Map<String, Class<?>> accumulator = new HashMap<String, Class<?>>();
        collectInterfaces(clazz, accumulator);
        return accumulator;
    }

    private static void collectInterfaces(final Class<?> clazz, final Map<String, Class<?>> accumulator) {
