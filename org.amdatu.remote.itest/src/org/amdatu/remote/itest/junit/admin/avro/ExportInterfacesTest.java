...
public class ExportInterfacesTest extends AbstractRemoteServiceAdminTest {
    ...
    /**
     * Remote Service Admin 122.5.1 <br/>
     * <br/>
     * service.exported.interfaces – (String+) This property must be set; it
     * marks this service for export and defines the interfaces.<br/>
     * The list members must all be contained in the types listed in the
     * objectClass service property from the Service Reference. The single value
     * of an asterisk (’*’, \u002A) indicates all interfaces in the
     * registration’s objectClass property and ignore the classes. Being able
     * to set this property outside the Service Reference implies that the
     * Topology Manager can export any registered service, also services not
     * specifically marked to be exported.
     */
    ...
    /**
     * Tests that we cannot export a service that does not implement an exported
     * interface.
     */
    ...
    /**
     * Tests that trying to export a service without at least one exported
     * interface fails with an exception.
     */
    ...
    protected void doTestRsaHandlesExportedInterfaceWithEmptyExportPropertyFail() throws Exception {
        ...
        assertExportRegistrationFails(serviceProperties, null, IllegalArgumentException.class, createEchoInstance(),
            ExtendedEchoInterface.class);
    }

    /**
     * Tests that we can export a service that implements a subtype of the
     * exported interface.
     */
    ...
    /**
     * Tests that we can export a service that implements multiple exported
     * interface.
     */
    protected void doTestRsaHandlesMultipleExportedInterfacesOk() throws Exception {
        ...
        serviceProperties.put(SERVICE_EXPORTED_INTERFACES, strings(EchoInterface.class.getName(),
            ExtendedEchoInterface.class.getName()));
    ...
    /**
     * Tests that exporting all interfaces implemented by a service by using a
    ...
    /**
     * Tests that exporting a wildcard as exported interface can not occur
     * together with other interface names.
     */
    protected void doTestRsaHandlesWildcardMixedWithExportedInterfaceFail() throws Exception {
        ...
        assertExportRegistrationFails(serviceProperties, null, IllegalArgumentException.class,
            createExtendedEchoInstance(), ExtendedEchoInterface.class);
    }
